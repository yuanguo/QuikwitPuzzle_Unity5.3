...
public class EventDelegate
{
    ...
    void Cache()
    {
        ...
        if (mCachedCallback == null || (mCachedCallback.Target as MonoBehaviour) != mTarget || GetMethodName(mCachedCallback) != mMethodName)
        {
            if (mTarget != null && !string.IsNullOrEmpty(mMethodName))
            {
                ...
                if (type == null)
                {
                    Debug.LogError("Do not exist " + mTarget.name + "'s Type");
                    return;
                }
                ...
                if (mMethod == null)
                {
					Debug.LogError("Could not find method '" + mMethodName + "' on " + mTarget.GetType());
                    return;
                }
    ...
    public void CopyParams(Parameter[] _params)
    {
        if (_params != null && _params.Length > 0)
        {
            mParameters = new Parameter[_params.Length];
            for (int i = 0; i < _params.Length; ++i)
                mParameters[i] = _params[i];
        }
    }
    ...
    static public void Add(List<EventDelegate> list, Callback callback, bool oneShot)
    {
        ...
        {
            Debug.LogError("Attempting to add a callback to a list that's null");
        }
    }
